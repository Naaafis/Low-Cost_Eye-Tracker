# python itracker_adv.py -i /Users/cheffbcookin/Desktop/EC544/Eye-Tracker/eye_tracker_train_and_val.npz -lm /Users/cheffbcookin/Desktop/EC544/Eye-Tracker/pretrained_models/itracker_adv/model-23.meta
...
class EyeTracker(object):
    ...
    def train(self, train_data, val_data, lr=1e-3, batch_size=128, max_epoch=1000, min_delta=1e-4, patience=10, print_per_epoch=10, out_model='my_model'):
        ...
        print(f"Train on {train_data[0].shape[0]} samples, validate on {val_data[0].shape[0]} samples")
        ...
        with tf.Session() as sess:
            ...
            for n_epoch in range(1, max_epoch + 1):
                ...
                if n_epoch % print_per_epoch == 0:
                    print(f"Epoch {n_epoch}/{max_epoch}, train loss: {train_loss:.5f}, train error: {train_err:.5f}, val loss: {val_loss:.5f}, val error: {val_err:.5f}")
                    #'Epoch %s/%s, train loss: %.5f, train error: %.5f, val loss: %.5f, val error: %.5f' % \
                ...
                if n_incr_error >= patience:
                    ...
                    #'Early stopping occured. Optimization Finished!'
...
def load_model(session, save_path):
    ...
    """
    print(f'Loading model from file {save_path}...')
    #"Loading model from file '%s'..." % save_path
...
def test(args):
    # data = np.load('/Users/cheffbcookin/Desktop/EC544/Eye-Tracker/eye_tracker_train_and_val.npz')
    # print(data.keys())
    # inp = data[args.input]
    
    # if args.input == 'train_data':
    #     _, val_data = load_data(train_data)
    # else:
    ...
    with tf.compat.v1.Session() as sess:
        ...
        print('Overall validation error: {}'.format(error))
        #'Overall validation error: %f' % error
